...
from Training.Externals.Loss_Diagram_Values import (
    plot_loss_curves_FineTuning_script_,
    visualize_and_save_waveforms,
    evaluate_metrics_from_spectrograms
)
from Training.Externals.Functions import load_model_path_func, freeze_encoder
...
loss_history_finetuning_epoches = {
    "l1": [], "mse": [], "spectral": [], "perceptual": [], "multiscale": [], "combined": []
}

# Create dataloaders
Fine_tuned_training_loader, Finetuned_validation_loader = create_dataloader_Fine_tuning(
...
def fine_tune_model(fine_tuned_model_path, training_loader, validation_loader, model_engine, scheduler, fine_tune_epochs=10, pretrained_model_path=None):
    ...
    try:
        for epoch in range(fine_tune_epochs):
            ...
            for batch_idx, (inputs, targets) in enumerate(training_loader, start=1):
                ...
                Fine_tune_logger.info(
                    f"Epoch [{epoch+1}], Batch [{batch_idx}] -> Combined Loss: {combined_loss.item():.6f}, "
                    f"Mask Loss: {mask_loss.item():.6f}, Total Loss: {total_loss.item():.6f}"
                )
            ...
            with torch.no_grad():
                ...
                for val_batch_idx, (inputs, targets) in enumerate(validation_loader, start=1):
                    if inputs is None or targets is None:
...
if __name__ == "__main__":
    ...
    fine_tune_model(
        ...
        training_loader=Fine_tuned_training_loader,
        validation_loader=Finetuned_validation_loader,
